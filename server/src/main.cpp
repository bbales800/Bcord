...
// ===== GLOBALS =======================================================================
struct RedisEnv { std::unique_ptr<sw::redis::Redis> conn; bool enabled=false; } g_redis;
struct PgEnv    { std::unique_ptr<pqxx::connection>  conn; bool enabled=false; } g_pg;

static std::string now_stamp() {
    using clock = std::chrono::system_clock;
    auto t = clock::to_time_t(clock::now());
    std::tm tm{}; localtime_r(&t, &tm);
    std::ostringstream os; os << std::put_time(&tm, "%Y-%m-%d %H:%M:%S");
    return os.str();
}
static std::string make_session_id() {
    static thread_local std::mt19937_64 rng{std::random_device{}()};
    static const char* hex = "0123456789abcdef";
    std::string s(16,'0'); for (auto& c: s) c = hex[rng() & 0xF]; return s;
}
static std::string random_token(size_t n=48) {
    static thread_local std::mt19937_64 rng{std::random_device{}()};
    static const char* base = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    std::string s; s.reserve(n); for (size_t i=0;i<n;i++) s.push_back(base[rng()%62]); return s;
}
...
struct ChannelRegistry {
    ...
    std::vector<std::pair<long,std::string>> users_in(const std::string& ch) const;
...
static void pg_init_auth_schema() {
    ...
    g_pg.conn->prepare("user_by_email",
        "SELECT id,email,username,pw_hash FROM users WHERE email=$1");
    g_pg.conn->prepare("user_by_name",
        "SELECT id,email,username,pw_hash FROM users WHERE username=$1");
    g_pg.conn->prepare("user_by_id",
        "SELECT username FROM users WHERE id=$1");
    g_pg.conn->prepare("user_insert",
        "INSERT INTO users(email,username,pw_hash) VALUES($1,$2,$3) RETURNING id");

    g_pg.conn->prepare("session_select_valid",
        "SELECT user_id FROM sessions WHERE token=$1 AND expires_at > now()");
    g_pg.conn->prepare("session_insert_hours",
        "INSERT INTO sessions(token,user_id,expires_at) VALUES($1,$2, now() + make_interval(hours => $3::int))");
    g_pg.conn->prepare("session_delete",
...
static void pg_init_channel_schema() {
    ...
    g_pg.conn->prepare("unsubscribe_update",
    ...
    g_pg.conn->prepare("channels_for_guild",
      "SELECT id, guild_id, parent_id, kind::text AS kind, name, position, is_private "
      "FROM channels WHERE guild_id=$1 ORDER BY position, id");

    g_pg.conn->prepare("guild_insert",
      "INSERT INTO guilds(name,created_by) VALUES($1,$2) RETURNING id");

    g_pg.conn->prepare("guild_member_insert",
      "INSERT INTO guild_members(guild_id,user_id,nick) VALUES($1,$2,$3) ON CONFLICT DO NOTHING");
...
static void pg_init_msg_schema() {
    ...
    // HISTORY prepares (by channel_id only)
...
pg_find_user_by_email(const std::string& email){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("user_by_email",email);
...
pg_find_user_by_name(const std::string& u){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("user_by_name",u);
    if (r.empty()) return std::nullopt;
    return std::make_tuple(r[0]["id"].as<long>(), r[0]["email"].c_str(),
                           r[0]["username"].c_str(), r[0]["pw_hash"].c_str());
}
static std::optional<std::string> pg_username_by_id(long uid){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("user_by_id",uid);
    if (r.empty()) return std::nullopt; return std::string(r[0]["username"].c_str());
}
static long pg_insert_user(const std::string& email,const std::string& user,const std::string& hash){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("user_insert",email,user,hash); long id=r[0]["id"].as<long>(); tx.commit(); return id;
}
static std::string pg_create_session(long uid,int hours=SESSION_HOURS_DEFAULT){
    std::string token=random_token(); pqxx::work tx{*g_pg.conn};
    tx.exec_prepared("session_insert_hours",token,uid,hours); tx.commit(); return token;
}
static std::optional<long> pg_validate_session(const std::string& token){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("session_select_valid",token);
    if (r.empty()) return std::nullopt; return r[0]["user_id"].as<long>();
}
static long pg_channel_id_by_name(const std::string& name){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("channel_id_by_name",name);
    if (r.empty()) return 0; return r[0]["id"].as<long>();
}
static std::optional<std::string> pg_channel_name_by_id(long id){
    pqxx::work tx{*g_pg.conn}; auto r=tx.exec_prepared("channel_name_by_id",id);
    if (r.empty()) return std::nullopt; return std::string(r[0]["name"].c_str());
}
static void pg_subscribe(long chan_id,long uid,bool on){
    pqxx::work tx{*g_pg.conn};
    if (on) tx.exec_prepared("subscribe_upsert",chan_id,uid);
...
static json pg_guilds_for_user(long uid) {
    ...
    for (auto const& row : r) {
        json g; g["id"]=row["id"].as<long>(); g["name"]=row["name"].c_str();
        arr.push_back(g);
    }
    return arr;
}
static json pg_channels_for_guild(long gid) {
    ...
    for (auto const& row : r) {
        ...
        if (!row["parent_id"].is_null()) c["parent_id"]=row["parent_id"].as<long>();
        ...
        if (kind=="category") cats.push_back(c);
        else if (kind=="text") text.push_back(c);
        else voice.push_back(c);
    }
    json out; out["categories"]=cats; out["text"]=text; out["voice"]=voice; return out;
}
static long pg_guild_create(const std::string& name, long owner, const std::string& nick) {
    pqxx::work tx{*g_pg.conn};
...
static long pg_channel_create(long gid,
    ...
    pqxx::work tx{*g_pg.conn};
    auto r = tx.exec_prepared("channel_insert",
        gid,
...
static void redis_set_online(long uid){
    if (!g_redis.enabled) return;
    try { g_redis.conn->setex("online:user:"+std::to_string(uid), ONLINE_TTL_SECONDS, "1"); } catch(...) {}
}
static void redis_room_add(long chan_id,long uid){
    if (!g_redis.enabled) return;
    try { g_redis.conn->sadd("presence:chan:"+std::to_string(chan_id), std::to_string(uid)); } catch(...) {}
}
static void redis_room_del(long chan_id,long uid){
    if (!g_redis.enabled) return;
    try { g_redis.conn->srem("presence:chan:"+std::to_string(chan_id), std::to_string(uid)); } catch(...) {}
}
static long redis_room_count(long chan_id){
    if (!g_redis.enabled) return -1;
    try { return g_redis.conn->scard("presence:chan:"+std::to_string(chan_id)); } catch(...) { return -1; }
}
static std::vector<long> redis_room_members(long chan_id){
    std::vector<long> out; if (!g_redis.enabled) return out;
    try {
        std::vector<std::string> members;
        g_redis.conn->smembers("presence:chan:"+std::to_string(chan_id), std::back_inserter(members));
        out.reserve(members.size());
        for (const auto& s : members) { try { out.push_back(std::stol(s)); } catch(...) {} }
    } catch(...) {}
    return out;
}

// ===== MESSAGES ======================================================================
static long long pg_insert_message_id(long gid, long cid, const std::string& user,
    ...
    tx.commit();
    std::cerr << "[msg] inserted message_id=" << id << " cid=" << cid << " user=" << user << "\n";
    return id;
}
...
struct Session {
    ...
    explicit Session(tcp::socket sock) : ws(std::move(sock)) {
        ...
        remote_addr = ep.address(); remote_port = ep.port();
    }
    void send(const json& j) {
        std::scoped_lock lk(write_mu);
        auto s = j.dump(); ws.text(true); ws.write(net::buffer(s));
    }
    void join_key(const std::string& key) {
        if (joined.insert(key).second) g_channels.join(key, this);
    }
    void leave_key(const std::string& key) {
        if (joined.erase(key)) g_channels.leave(key, this);
    }
    void leave_all(){
        for (const auto& key : joined) {
            if (key.rfind("chan#",0)==0) {
                ...
                g_channels.broadcast(key, {{"op","left"},{"channel_id",id},{"user",nick},{"user_id",user_id}});
                json users=json::array();
                for (auto uid : redis_room_members(id)) if (auto u=pg_username_by_id(uid)) users.push_back({{"user_id",uid},{"nick",*u}});
                g_channels.broadcast(key, {{"op","presence_list_ok"},{"channel_id",id},{"users",users}});
            }
...
void ChannelRegistry::join(const std::string& ch, Session* s){
    std::scoped_lock lk(mu); map[ch].insert(s);
}
...
std::vector<std::pair<long,std::string>> ChannelRegistry::users_in(const std::string& ch) const {
    std::vector<std::pair<long,std::string>> out;
    std::scoped_lock lk(mu);
    auto it = map.find(ch);
    if (it == map.end()) return out;
    out.reserve(it->second.size());
    for (auto* s : it->second) out.push_back({s->user_id, s->nick});
    return out;
}
void ChannelRegistry::broadcast(const std::string& ch, const json& msg) {
    std::vector<Session*> targets;
    {
        ...
        for (auto* s : it->second) targets.push_back(s);
    }
    for (auto* s : targets) { try { s->send(msg); } catch (...) {} }
}
static void broadcast_roster_snapshot(long chan_id){
    auto key = chkey(chan_id);
    json users=json::array();
    for (auto uid : redis_room_members(chan_id)) if (auto u=pg_username_by_id(uid)) users.push_back({{"user_id",uid},{"nick",*u}});
    g_channels.broadcast(key, {{"op","presence_list_ok"},{"channel_id",chan_id},{"users",users}});
}

// ===== SOCKET LOOP ===================================================================
void session_loop(tcp::socket socket) {
    auto sp = std::make_shared<Session>(std::move(socket));
    const auto who = sp->remote_addr.to_string() + ":" + std::to_string(sp->remote_port);
    ...
    try {
        ...
        while (true) {
            ...
            json in;
            try { in = json::parse(text); }
            catch (...) { sp->send({{"op","error"},{"reason","invalid_json"}}); continue; }

            const auto op = in.value("op", "");

            // -------- REGISTER --------
            if (op == "register") {
                if (!g_pg.enabled) { sp->send({{"op","error"},{"reason","pg_disabled"}}); continue; }
                std::string email=in.value("email",""), user=in.value("username",""), pw=in.value("password","");
                if (email.empty()||user.empty()||pw.empty()) { sp->send({{"op","error"},{"reason","missing_fields"}}); continue; }
                if (pg_find_user_by_email(email) || pg_find_user_by_name(user)) { sp->send({{"op","error"},{"reason","user_exists"}}); continue; }
                uint32_t t_cost=2, m_cost=1<<16, parallelism=1;
                std::array<uint8_t,16> salt{}; for (auto& b: salt) b = uint8_t(std::rand() & 0xFF);
                char enc[argon2_encodedlen(t_cost,m_cost,parallelism,salt.size(),32,Argon2_id)];
                if (argon2id_hash_encoded(t_cost,m_cost,parallelism,pw.data(),pw.size(),salt.data(),salt.size(),32,enc,sizeof(enc)) != ARGON2_OK) {
                    sp->send({{"op","error"},{"reason","hash_failed"}}); continue;
                }
                long uid = pg_insert_user(email,user,enc);
                std::string token = pg_create_session(uid);
                sp->authenticated=true; sp->user_id=uid; sp->nick=user;
                sp->send({{"op","register_ok"},{"token",token},{"user_id",uid},{"nick",sp->nick}});
                sp->send({{"op","auth_ok"},{"user_id",uid},{"nick",sp->nick}});
            }
            // -------- LOGIN --------
            else if (op == "login") {
                if (!g_pg.enabled) { sp->send({{"op","error"},{"reason","pg_disabled"}}); continue; }
                std::string ident=in.value("id", in.value("username", in.value("email","")));
                std::string pw=in.value("password","");
                if (ident.empty()||pw.empty()) { sp->send({{"op","error"},{"reason","missing_fields"}}); continue; }
                auto u = (ident.find('@')!=std::string::npos) ? pg_find_user_by_email(ident) : pg_find_user_by_name(ident);
                if (!u) { sp->send({{"op","error"},{"reason","not_found"}}); continue; }
                auto [uid,email,uname,pwhash] = *u;
                if (argon2id_verify(pwhash.c_str(), pw.data(), pw.size()) != ARGON2_OK) {
                    sp->send({{"op","error"},{"reason","bad_password"}}); continue;
                }
                std::string token = pg_create_session(uid);
                sp->authenticated=true; sp->user_id=uid; sp->nick=uname;
                sp->send({{"op","login_ok"},{"token",token},{"user_id",uid},{"nick",sp->nick}});
                sp->send({{"op","auth_ok"},{"user_id",uid},{"nick",sp->nick}});
            }
            // -------- AUTH (token) --------
            else if (op == "auth") {
                ...
                if (token.empty()) { sp->send({{"op","error"},{"reason","missing_token"}}); continue; }
                auto uid = pg_validate_session(token);
                if (!uid) { sp->send({{"op","error"},{"reason","token_invalid"}}); continue; }
                auto uname = pg_username_by_id(*uid).value_or("user"+std::to_string(*uid));
                sp->authenticated=true; sp->user_id=*uid; sp->nick=uname;
                sp->send({{"op","auth_ok"},{"user_id",*uid},{"nick",sp->nick}});
            }
            // -------- GUILDS --------
                ...
                if (name.empty()) { sp->send({{"op","error"},{"reason","missing_name"}}); continue; }
            ...
            // -------- CHANNELS (per guild) --------
                ...
                if (!gid) { sp->send({{"op","error"},{"reason","bad_guild"}}); continue; }
                ...
                if (!sp->authenticated) { sp->send({{"op","error"},{"reason","auth_required"}}); continue; }
                long gid = in.value("guild_id", 0L);
                std::string kind = in.value("kind","text"); // 'text'|'category'|'voice'
                std::string name = in.value("name","");
                int position = in.value("position", 0);
                std::optional<long> parent;
                if (in.contains("parent_id")) parent = in["parent_id"].get<long>();
                if (!gid || name.empty()) { sp->send({{"op","error"},{"reason","missing_fields"}}); continue; }
            ...
            // ---- CHANNEL: rename -------------------------------------------------------
                ...
                // (Optional) check: user must be guild member — we skip advanced perms for now
            ...
            // -------- JOIN / LEAVE --------
                ...
                if (!sp->authenticated) { sp->send({{"op","error"},{"reason","auth_required"}}); continue; }
                long cid = in.value("channel_id", 0L);
                if (!cid) { std::string name = in.value("channel","general"); cid = pg_channel_id_by_name(name); }
                if (!cid || !pg_channel_is_joinable(cid, sp->user_id)) { sp->send({{"op","error"},{"reason","not_allowed"}}); continue; }
                auto key = chkey(cid);
                sp->join_key(key);
                redis_room_add(cid, sp->user_id);
                g_channels.broadcast(key, {{"op","joined"},{"channel_id",cid},{"user",sp->nick},{"user_id",sp->user_id}});
                // roster to joiner
                json users=json::array();
                for (auto uid : redis_room_members(cid)) if (auto u=pg_username_by_id(uid)) users.push_back({{"user_id",uid},{"nick",*u}});
                std::string cname = pg_channel_name_by_id(cid).value_or("general");
                sp->send({{"op","joined_ok"},{"channel_id",cid},{"name",cname},
                          {"you", {{"user_id",sp->user_id},{"nick",sp->nick}}},{"users",users}});
                broadcast_roster_snapshot(cid);
                // proactive latest history
                auto rows = pg_hist_latest(cid, HISTORY_PAGE_DEFAULT);
                json items = json::array();
                for (auto& r : rows) { json it; it["message_id"]=r.message_id; it["username"]=r.user; it["body"]=r.text; it["ts"]=r.ts; items.push_back(it); }
                sp->send({{"op","history_ok"},{"mode","latest"},{"channel_id",cid},{"items",items}});
            }
            else if (op == "leave") {
                long cid = in.value("channel_id", 0L);
                auto key = chkey(cid);
                ...
                g_channels.broadcast(key, {{"op","left"},{"channel_id",cid},{"user",sp->nick},{"user_id",sp->user_id}});
                broadcast_roster_snapshot(cid);
            ...
            // -------- MSG --------
                ...
                if (cid <= 0 || text.empty()) continue;
                // derive guild_id by lookup from channels
                long gid = 0;
                try {
                    pqxx::work tx{*g_pg.conn};
                    auto r = tx.exec_params("SELECT guild_id FROM channels WHERE id=$1", cid);
            ...
            // -------- HISTORY (before) --------
                ...
                json items = json::array();
                for (auto& r : rows) { json it; it["message_id"]=r.message_id; it["username"]=r.user; it["body"]=r.text; it["ts"]=r.ts; items.push_back(it); }
                sp->send({{"op","history_ok"},{"mode","before"},{"channel_id",cid},{"items",items}});
            }
            // -------- HISTORY (since) --------
                ...
                json items = json::array();
                for (auto& r : rows) { json it; it["message_id"]=r.message_id; it["username"]=r.user; it["body"]=r.text; it["ts"]=r.ts; items.push_back(it); }
                sp->send({{"op","history_ok"},{"mode","since"},{"channel_id",cid},{"items",items}});
            }
            // -------- PING --------
            else if (op == "ping") {
                if (sp->authenticated) redis_set_online(sp->user_id);
                sp->send({{"op","pong"}});
            }
            // -------- UNKNOWN --------
            else {
        ...
        std::cerr << "[" << now_stamp() << "] disconnect " << who << " sid=" << sp->sid << " reason=" << se.what() << "\n";
    } catch (const std::exception& ex) {
        std::cerr << "[" << now_stamp() << "] disconnect " << who << " sid=" << sp->sid << " fatal=" << ex.what() << "\n";
    }
    sp->leave_all();
}
...
int main() {
    try {
        ...
        unsigned short port   = static_cast<unsigned short>(env_port ? std::atoi(env_port) : 9000);

        // Redis
        if (const char* r = std::getenv("REDIS_URL"); r && *r) {
            try { g_redis.conn = std::make_unique<sw::redis::Redis>(std::string(r)); g_redis.enabled=true; std::cerr<<"["<<now_stamp()<<"] Redis connected\n"; }
            catch(const std::exception& ex){ std::cerr<<"["<<now_stamp()<<"] Redis disabled: "<<ex.what()<<"\n"; g_redis.enabled=false; }
        } else std::cerr<<"["<<now_stamp()<<"] Redis disabled (REDIS_URL not set)\n";

        // Postgres
        try {
            const char *h=getenv("PGHOST"), *db=getenv("PGDATABASE"), *u=getenv("PGUSER"), *p=getenv("PGPASSWORD"), *po=getenv("PGPORT");
            if (h&&db&&u&&p) {
                std::string conn = "host="+std::string(h)+" dbname="+db+" user="+u+" password="+p+(po?(" port="+std::string(po)):"");
                g_pg.conn = std::make_unique<pqxx::connection>(conn); g_pg.enabled = g_pg.conn->is_open();
                if (g_pg.enabled) {
                    std::cerr<<"["<<now_stamp()<<"] Postgres connected\n";
                    pg_init_auth_schema();
                    pg_init_channel_schema();
                    pg_init_msg_schema();
                } else std::cerr<<"["<<now_stamp()<<"] Postgres disabled (not open)\n";
            } else std::cerr<<"["<<now_stamp()<<"] Postgres disabled (env not set)\n";
        } catch(const std::exception& ex){ std::cerr<<"["<<now_stamp()<<"] Postgres disabled: "<<ex.what()<<"\n"; g_pg.enabled=false; }
        ...
        while (!ioc.stopped()) {
            ...
            if (ec) { std::cerr<<"["<<now_stamp()<<"] accept error: "<<ec.message()<<"\n"; std::this_thread::sleep_for(std::chrono::milliseconds(50)); continue; }
            std::thread(session_loop, std::move(s)).detach();
        }
        return 0;
    } catch(const std::exception& ex){ std::cerr<<"["<<now_stamp()<<"] Fatal: "<<ex.what()<<"\n"; return 1; }
}
